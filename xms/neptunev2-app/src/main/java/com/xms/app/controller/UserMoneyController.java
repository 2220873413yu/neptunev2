...
/*	*//**
	 ...
	 * @return
	 *//*
	@ApiOperation(value = "资产划转从今日总动态收益划转到提现余额")
	@GetMapping(value = "transferBalance")
	public ResultPista transferBalance() {
		return ResultPista.data(bizUserMoneyService.transferBalance(SecurityUtils.getLoginAppUser().getUserId()));
	}*/
	...
	public ResultPista<PageInfo<UserMoneyLogBo>> getUserMoneyLogList(
		...
		@ApiParam(value = "币种") @RequestParam(required = false) String coinType,
		@ApiParam(value = "来源类型") @RequestParam(required = false) String sourceType) {
