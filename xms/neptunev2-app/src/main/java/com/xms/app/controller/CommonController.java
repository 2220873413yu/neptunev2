...
import com.xms.app.entity.ReferralRewardDto;
...
import com.xms.dao.service.impl.W3UserLevelLogServiceImpl;
...
public class CommonController {
	private static List<Integer> typeList = Arrays.asList(0,1,2,3);
	@Autowired
	...
	@Autowired
	private XmsCommonService xmsCommonService;

	@Autowired
	private ServerConfig serverConfig;
	@Autowired
	private IW3UserLevelConfigService w3UserLevelLogService;
...
//	}
	...
	/**
	 * 获取代币价格
	 * @param tokenName 代币名称：h、acp
	 * @return 返回代币价格
	 */
	@ApiOperation(value = "获取代币价格")
	@GetMapping(value = "/price")
	@Anonymous
	public ResultPista<Map<String, Object>> getTokenPrice(
		@ApiParam(value = "代币名称：h、acp", required = true) @RequestParam String tokenName) {
		if (tokenName == null || tokenName.trim().isEmpty()) {
			return ResultPista.fail("代币名称不能为空");
		}
		String token = tokenName.trim().toLowerCase(Locale.ROOT);
		BigDecimal price;
		if ("h".equals(token)) {
			price = xmsTokenPriceService.getHPriceUsdt();
		} else if ("acp".equals(token)) {
			price = xmsTokenPriceService.getAcpPriceUsdt();
		} else {
			return ResultPista.fail("不支持的代币名称");
		}
...
//	/**
//	 * 查询最新的3条公告
//	 * @param locale 语言 1:繁体中文,2:英文,3:日文,4:韩文,5:泰文,6:越南语
//	 * @return
//	 */
...
//	 * @param locale 协议类型  1:繁体中文,2:英文,3:日文,4:韩文,5:泰文,6:越南语
//	 * @return 返回协议信息
//	 */
...
//	}
	...
	/**
	 * 能否下质押单 true能下单,false不能下单
	 * @return
	 */
	@ApiOperation(value = "能否下质押单")
	@GetMapping(value = "/cpsor")
	@Anonymous
	public ResultPista<Boolean> canPlaceStakeOrder() {
		//如果在每天的23:59:00-23:59:59之间，则不能下单
		Date now = new Date();
		...
		ResultPista resultPista = xmsCommonService.checkMineSettleTime();
