...
public class UserInfoController extends BaseController {
	 ...
	 * 获取用户今日购买统计
	 */
	@PreAuthorize("@ss.hasPermi('xms:userinfo:todayBuy')")
	@GetMapping("/todayBuyStatistics")
	public AjaxResult getTodayBuyStatistics(@RequestParam(required = false) List<Long> userIds) {
		// 如果没有指定用户ID，则查询所有用户
		if (CollectionUtil.isEmpty(userIds)) {
			// 获取所有用户ID（可以根据需要添加条件限制）
			List<UserInfo> allUsers = xmsUserInfoService.selectUserInfoList(new UserInfo());
			userIds = allUsers.stream().map(UserInfo::getUserId).collect(Collectors.toList());
		}

		// 查询今日购买记录
		List<MiningPackageOrder> todayOrders = miningPackageOrderService.lambdaQuery()
			.in(MiningPackageOrder::getUserId, userIds)
			.eq(MiningPackageOrder::getPrincipalReturned, 0)
			.apply(" create_time >=CURDATE() ")
			.select(MiningPackageOrder::getUserId, MiningPackageOrder::getBuyPrice)
			.list();

		// 使用stream统计每个用户今日购买总金额
		Map<Long, BigDecimal> userTodayBuyAmount = todayOrders.stream()
			.collect(Collectors.groupingBy(
				MiningPackageOrder::getUserId,
				Collectors.reducing(
					BigDecimal.ZERO,
					MiningPackageOrder::getBuyPrice,
					BigDecimal::add
				)
			));

		return AjaxResult.success(userTodayBuyAmount);
	}
