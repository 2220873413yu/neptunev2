...
public class XmsUserInfoServiceImpl implements XmsUserInfoService {
	...
	public AjaxResult updateUserInfo(UserInfoReqBo req) {
		...
		updateUser.setMinNodeLevel(req.getMinNodeLevel());
		updateUser.setMinLayerLevel(req.getMinLayerLevel());
	...
	public List<Tree<Long>> queryNetBody1(String userId) {
		...
		if(CollectionUtil.isEmpty(userList)){
			...
			return TreeBuildUtils.build(userList, ((user, tree) -> {
				...
				//等级
				tree.putExtra("level", levelMap.get(user.getGameLevel()));
				//节点等级
				tree.putExtra("nodeLevel", nodeLevelMap.get(user.getNodeLevel()));
				//个人业绩(节点数量)
				tree.putExtra("performance", user.getPerformance());
				//直推业绩(节点数量)
				tree.putExtra("subPerformance", user.getSubPerformance());
				//团队业绩(节点数量)
				tree.putExtra("umbrellaPerformance", user.getUmbrellaPerformance());
				//直推用户
				tree.putExtra("subNum", user.getSubNum());
				//直推有效用户数量
				tree.putExtra("validSubNum", user.getValidSubNum());
				//用户编码
				tree.putExtra("userCode", user.getUserCode());
				//团队用户数
				tree.putExtra("umbrellaNum", user.getUmbrellaNum());
				//团队有效用户数量
				tree.putExtra("validUmbrellaNum", user.getValidUmbrellaNum());
				//tree.putExtra("teamRewardRatio", BigDecimal.ZERO);
			}));
		}
		userList.addFirst(currentUser);
