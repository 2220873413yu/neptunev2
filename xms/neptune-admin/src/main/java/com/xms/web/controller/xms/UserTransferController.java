...
public class UserTransferController extends BaseController
{
    ...
    {
		if (!StrUtil.isBlank(userTransfer.getFromUserCode())) {
			UserInfo userInfo = userInfoServiceImpl.lambdaQuery()
				.eq(UserInfo::getAccount, userTransfer.getFromUserCode())
				.select(UserInfo::getUserId)
				.one();
			if(userInfo ==null){
				return getDataTable(new ArrayList<>());
			}
			userTransfer.setFromUserId(userInfo.getUserId());
		}

		if (!StrUtil.isBlank(userTransfer.getToUserCode())) {
			UserInfo userInfo = userInfoServiceImpl.lambdaQuery()
				.eq(UserInfo::getAccount, userTransfer.getToUserCode())
				.select(UserInfo::getUserId)
				.one();
			if(userInfo ==null){
				return getDataTable(new ArrayList<>());
			}
			userTransfer.setToUserId(userInfo.getUserId());
		}
        startPage();
        List<UserTransfer> list = userTransferService.selectUserTransferList(userTransfer);

		if(CollectionUtil.isNotEmpty(list)){
			Map<Long, String> fromUserAccountMap = userInfoServiceImpl.getUserAccountById(list.stream().map(UserTransfer::getFromUserId).collect(Collectors.toList()));
			Map<Long, String> toUserAccountMap = userInfoServiceImpl.getUserAccountById(list.stream().map(UserTransfer::getToUserId).collect(Collectors.toList()));
			for (UserTransfer bankVo : list) {
				bankVo.setFromUserCode(fromUserAccountMap.get(bankVo.getFromUserId()));
				bankVo.setToUserCode(toUserAccountMap.get(bankVo.getToUserId()));
			}
		}
    ...
    public void export(HttpServletResponse response, UserTransfer userTransfer)
    {
		if (!StrUtil.isBlank(userTransfer.getFromUserCode())) {
			...
			if(userInfo ==null){
				...
				util.exportExcel(response, new ArrayList<>(), "用户转账记录数据");
			}
			userTransfer.setFromUserId(userInfo.getUserId());
		}
