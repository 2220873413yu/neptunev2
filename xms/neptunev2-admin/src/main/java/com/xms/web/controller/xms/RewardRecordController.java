...
@GetMapping("/list")
    public TableDataInfo list(RewardRecord rewardRecord)
    {
		...
		if(CollectionUtil.isNotEmpty(list)){
			Map<Long, String> userAccountMap = userInfoService.getUserAccountById(list.stream().map(RewardRecord::getUserId).collect(Collectors.toList()));
			for (RewardRecord order : list) {
				order.setUserAccount(userAccountMap.get(order.getUserId()));
			}
		}
