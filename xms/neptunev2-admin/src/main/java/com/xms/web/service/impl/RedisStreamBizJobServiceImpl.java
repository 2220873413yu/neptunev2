...
//					.set(UserInfo::getCommunityPerformance,0)
	...
	public Integer handleBizType1(OrderMsgDO orderMsgDO) {
		...
		//计算用户的层级
		calculateLayerLevel(userInfo.getEffectiveHistoryPerformance(), userInfo);
			...
			if (StrUtil.isNotBlank(userInfo.getParentChain())) {
				...
				for (UserInfo parentUser : parentUserList) {
					...
					for (W3UserLevelConfig w3UserLevelConfig : userLevelConfigList) {
						//个人业绩
						if(parentUser.getEffectivePerformance().compareTo(w3UserLevelConfig.getPerformance())>=0){
							//团队业绩
							if(parentUser.getEffectiveUmbrellaPerformance().compareTo(w3UserLevelConfig.getUmbrellaPerformance())>=0){
