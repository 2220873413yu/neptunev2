...
public class UserMoneyLogServiceImpl extends ServiceImpl<UserMoneyLogMapper, UserMoneyLog> implements UserMoneyLogService {
	 ...
	 */
	@Override
	public PageInfo<UserMoneyLogBo> getUserMoneyLogList(Integer pageIndex, Integer pageSize, Long userId, String coinType,
														String sourceType) {
		List<Integer> coinTypeList = null;
		List<Integer> sourceTypeList = null;
		if (StringUtils.isNotEmpty(coinType)) {
			coinTypeList = Arrays.stream(coinType.split(","))
				.map(String::trim)
		...
		if (StringUtils.isNotEmpty(sourceType)) {
			sourceTypeList = Arrays.stream(sourceType.split(","))
				.map(String::trim)
				.filter(StringUtils::isNotEmpty)
				.map(Integer::valueOf)
				.collect(Collectors.toList());
		...
		PageInfo<UserMoneyLogBo> pageInfo = new PageInfo<>(this.baseMapper.getUserMoneyLogList(userId, coinTypeList, sourceTypeList));
		return pageInfo;
	}
