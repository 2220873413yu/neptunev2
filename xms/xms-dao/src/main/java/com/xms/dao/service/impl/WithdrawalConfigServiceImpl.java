...
public class WithdrawalConfigServiceImpl extends XmsDataServiceImpl<WithdrawalConfigMapper, WithdrawalConfig> implements IWithdrawalConfigService
{
	...
	public int updateByConfigById(WithdrawalConfig req) {
		...
		if(req.getWealthVaultRatio().compareTo(BigDecimal.ZERO)<0){
			throw new ServiceException("财富仓比例不能小于0");
		}

		if(req.getInsuranceVaultRatio().compareTo(BigDecimal.ZERO)<0){
			throw new ServiceException("保险仓比例不能小于0");
		}
		boolean update = lambdaUpdate()
			...
			.set(WithdrawalConfig::getWealthVaultRatio, req.getWealthVaultRatio())
			.set(WithdrawalConfig::getInsuranceVaultRatio, req.getInsuranceVaultRatio())
