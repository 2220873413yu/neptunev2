<template>
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="68px" size="small">
      ...
      <el-form-item label="购买的钱包地址" label-width="120px" prop="address">
        <el-input
          v-model="queryParams.address"
          clearable
          placeholder="请输入购买的钱包地址"
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
...
<!--      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xms:idoOrder:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['xms:idoOrder:export']"
    ...
    <el-table v-loading="loading" :data="idoOrderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="购买份数" prop="shares" />
      <el-table-column align="center" label="支付金额" prop="okbPaid" />
      <el-table-column align="center" label="交易hash" prop="txHash" />
<!--      <el-table-column label="时间戳 毫秒" align="center" prop="timestamp" />
...
<!--      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="购买份数" prop="shares">
          <el-input v-model="form.shares" placeholder="请输入购买份数" />
        </el-form-item>
        <el-form-item label="支付了多少金额" prop="okbPaid">
          <el-input v-model="form.okbPaid" placeholder="请输入支付了多少金额" />
        </el-form-item>
        <el-form-item label="交易hash" prop="txHash">
          <el-input v-model="form.txHash" placeholder="请输入交易hash" />
        </el-form-item>
        <el-form-item label="时间戳 毫秒" prop="timestamp">
          <el-input v-model="form.timestamp" placeholder="请输入时间戳 毫秒" />
        </el-form-item>
...
<script>
import { listIdoOrder, getIdoOrder, delIdoOrder, addIdoOrder, updateIdoOrder } from "@/api/xms/idoOrder";

export default {
  name: "IdoOrder",
  dicts: ['t_ido_order_biz_status'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      listIdoOrder(this.queryParams).then(response => {
        this.idoOrderList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateIdoOrder(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addIdoOrder(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除ido 订单编号为"' + ids + '"的数据项？').then(function() {
        return delIdoOrder(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xms/idoOrder/export', {
        ...this.queryParams
      }, `idoOrder_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
