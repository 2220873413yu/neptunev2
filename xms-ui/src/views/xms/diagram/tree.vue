      ...
      >
        <span slot-scope="{ node, data }" class="custom-tree-node">
          ...
          <span class="node-info">
            ...
            <el-tag size="mini" type="success">用户编码: {{ data.userCode }}</el-tag>
<!--            <el-tag size="mini" type="warning">等级: {{ data.level }}</el-tag>-->
            <el-tag size="mini" type="warning">节点等级: {{ data.nodeLevel }}</el-tag>
<!--            <el-tag size="mini" type="primary">个人业绩: {{ data.performance }}</el-tag>-->
            <el-tag size="mini" type="primary">直推节点: {{ data.subPerformance }}</el-tag>
            <el-tag size="mini" type="primary">团队节点: {{ data.umbrellaPerformance }}</el-tag>
      ...
      <div v-if="currentNode">
        <el-descriptions :column="1" border>
          <el-descriptions-item label="用户编码">
            {{ currentNode.userCode }}
          </el-descriptions-item>
          <el-descriptions-item label="用户账号">
            {{ currentNode.account }}
          </el-descriptions-item>
          <el-descriptions-item label="节点等级">
            {{ currentNode.nodeLevel }}
          </el-descriptions-item>
...
<!--          <el-descriptions-item label="个人业绩">
          ...
          <el-descriptions-item label="直推节点">
            {{ currentNode.subPerformance }}
          </el-descriptions-item>
          <el-descriptions-item label="团队节点">
            {{ currentNode.umbrellaPerformance }}
          </el-descriptions-item>
          <el-descriptions-item label="直推用户数">
            {{ currentNode.subNum }}
          </el-descriptions-item>
<!--          <el-descriptions-item label="直推有效用户数">
            {{ currentNode.validSubNum }}
          </el-descriptions-item>-->
          <el-descriptions-item label="团队用户数">
            {{ currentNode.umbrellaNum }}
          </el-descriptions-item>
<!--          <el-descriptions-item label="团队有效用户数">
            {{ currentNode.validUmbrellaNum }}
          </el-descriptions-item>-->
        </el-descriptions>
      </div>
  ...
  methods: {
    ...
    transformNodeData(node) {
      ...
      const transformedNode = {
        ...
        //团队收益
        //teamRewardRatio: node.teamRewardRatio,
        //直推业绩
        subPerformance: node.subPerformance,
        //团队业绩
        umbrellaPerformance: node.umbrellaPerformance,
        //直推用户数量
        subNum: node.subNum,
        //直推有效用户数量
        validSubNum: node.validSubNum,
        //团队用户数量
        umbrellaNum: node.umbrellaNum,
        //团队有效用户数量
        validUmbrellaNum: node.validUmbrellaNum,
        // 缓存子节点数据
