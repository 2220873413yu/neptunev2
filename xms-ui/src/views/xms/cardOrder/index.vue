<template>
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="68px" size="small">
      ...
      <el-form-item label="订单类型" label-width="120px" prop="orderType">
      ...
      <el-form-item label="卡片状态" label-width="120px" prop="status">
...
<!--      <el-form-item label="本次实际支付金额" prop="payAmount">
      ...
      <el-form-item label="卡片等级" label-width="150px" prop="cardType">
        <el-select v-model="queryParams.cardType" clearable placeholder="请选择卡片等级">
          <el-option
...
<!--      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xms:cardOrder:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['xms:cardOrder:export']"
...
<!--      <el-table-column type="selection" width="55" align="center" />-->
      ...
      <el-table-column align="center" label="卡片ID" prop="cardInstanceId" />
      <el-table-column align="center" label="主订单号" prop="masterOrderNo" />

      <el-table-column align="center" label="用户ID" prop="userId" />
      <el-table-column align="center" label="来源类型" prop="orderType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.card_order_type" :value="scope.row.orderType"/>
        </template>
      </el-table-column>
      <el-table-column align="center" label="套餐价格" prop="payAmount" >
        <template slot-scope="scope">
          <span>{{ scope.row.payAmount }} U</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="补贴算力" prop="computingPower">
      <template slot-scope="scope">
        <span>{{ scope.row.extraComputingPower }} T</span>
      </template>
      </el-table-column>
<!--      <el-table-column label="本次订单总算力" align="center" prop="currentComputingPower" />-->
      ...
      <el-table-column align="center" label="卡片状态" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.card_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column align="center" label="gtId" prop="orderNo" />
...
<!--      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="订单类型：1-购买新卡，2-补差价升级" prop="orderType">
          <el-select v-model="form.orderType" placeholder="请选择订单类型：1-购买新卡，2-补差价升级">
            <el-option
              v-for="dict in dict.type.card_order_type"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item label="算力" prop="computingPower">
          <el-input v-model="form.computingPower" placeholder="请输入算力" />
        </el-form-item>
...
<script>
import { listCardOrder, getCardOrder, delCardOrder, addCardOrder, updateCardOrder } from "@/api/xms/cardOrder";

export default {
  name: "CardOrder",
  dicts: ['card_order_type', 'card_type','card_status'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        orderType: [
          { required: true, message: "订单类型：1-购买新卡，2-补差价升级不能为空", trigger: "change" }
        ],
        payAmount: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除卡片订单编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xms/cardOrder/export', {
        ...this.queryParams
      }, `cardOrder_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
