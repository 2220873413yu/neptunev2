<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="用户ID" prop="id" label-width="120px">
        <el-input
...
<!--      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['xms:userWealthVault:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xms:userWealthVault:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xms:userWealthVault:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xms:userWealthVault:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="userWealthVaultList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="用户ID" align="center" prop="id" />
      <el-table-column label="钱包地址" align="center" prop="userAccount" />
      <el-table-column label="1段财富仓" align="center" prop="seg1Amount" />
      <el-table-column label="2段财富仓" align="center" prop="seg2Amount" />
      <el-table-column label="3段财富仓" align="center" prop="seg3Amount" />
      <el-table-column label="4段财富仓" align="center" prop="seg4Amount" />
      <el-table-column label="5段财富仓" align="center" prop="seg5Amount" />
...
<script>
import { listUserWealthVault, getUserWealthVault, delUserWealthVault, addUserWealthVault, updateUserWealthVault } from "@/api/xms/userWealthVault";

export default {
  name: "UserWealthVault",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUserWealthVault(this.queryParams).then(response => {
        this.userWealthVaultList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getUserWealthVault(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUserWealthVault(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addUserWealthVault(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户财富表编号为"' + ids + '"的数据项？').then(function() {
        return delUserWealthVault(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xms/userWealthVault/export', {
        ...this.queryParams
      }, `userWealthVault_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
